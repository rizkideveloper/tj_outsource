...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Data Koordinator Lapangan</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?= base_url('Halaman_utama'); ?>">Halaman utama</a></li>
            <li class="breadcrumb-item active" aria-current="page">Data Koordinator Lapangan</li>
        </ol>
    </div>
                        ...
                        <tbody>
                            ...
                            foreach ($korlap as $key => $value) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modal_ubah" onclick="tampil_id(<?= $value['id_korlap'] ?>)"><i class="fa fa-pencil-alt"></i></button>
                                        <button class="btn btn-danger btn-sm" onclick="hapus(<?= $value['id_korlap'] ?>)"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Korlap</h5>
            ...
            <div class="modal-body">
                <form id="form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="nama_korlap">
...
<div class="modal fade" id="modal_ubah" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Ubah Data Korlap</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form">
                    ...
                    <div class="form-group">
                        <label for="nama_korlap">Nama Korlap</label>
                        <input type="text" class="form-control" id="nama_korlap_edt">
...
<script>
    ...
    function tambah() {
        nik = $('#nik').val()
        nama_korlap = $('#nama_korlap').val()
        umur = $('#umur').val()
        perusahaan = $('#perusahaan').val()

        $.ajax({
            method: 'POST',
            url: '<?= base_url('Korlap/tambah')  ?>',
            data: {
                nik: nik,
            ...
            success: function(data) {
                if (data.status == 'gagal') {
                    ...
                    $('#nama_error').html(data.nama_korlap)
    ...
    function ubah() {
        id = $('#id_korlap_edt').val()
        nama_korlap = $('#nama_korlap_edt').val()
        umur = $('#umur_edt').val()
        nama_perusahaan = $('#perusahaan_edt').val()
        console.log(id, nama_korlap, nama_perusahaan, umur)
        $.ajax({
            method: 'POST',
            url: '<?= base_url('Korlap/ubah')  ?>',
            data: {
                id: id,
                nama_korlap: nama_korlap,
                umur: umur,
            ...
            success: function(data) {
                if (data.status == 'gagal') {
                    $('#nama_error_edt').html(data.nama_korlap)
