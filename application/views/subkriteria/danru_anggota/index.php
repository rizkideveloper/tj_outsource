...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Subkriteria Danru & Anggota</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="<?= base_url('Halaman_utama'); ?>">Halaman utama</a></li>
            <li class="breadcrumb-item">Data Subkriteria</li>
            <li class="breadcrumb-item active" aria-current="page">Data Subkriteria Danru & Anggota</li>
        </ol>
    </div>
        ...
        <div class="col-lg-12">
            <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row">
                    ...
                    <a class="btn btn-info btn-sm" href="<?= base_url('Subkriteria_danru_anggota/perbandingan_subkriteria'); ?>"><i class="fa fa-plus"></i> Perbandingan Subkriteria</a>
                </div>
                <div class="table-responsive p-3">
                    <table class="table align-items-center table-flush " id="dataTable">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            foreach ($subkriteria_da as $key => $value) { ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    <td><?= $value['kode_subkriteria_da']; ?></td>
                                    <td><?= $value['nama_subkriteria_da']; ?></td>
                                    <td><?= $value['bobot_subkriteria_da']; ?></td>
                                    <td><?= $value['nama_kriteria']; ?></td>
                                    <td>
                                        <button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modal_ubah" onclick="tampil_id(<?= $value['id_subkriteria_da'] ?>)"><i class="fa fa-pencil-alt"></i></button>
                                        <button class="btn btn-danger btn-sm" onclick="hapus(<?= $value['id_subkriteria_da'] ?>)"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
            ...
            <div class="modal-body">
                <form id="form">
                    ...
                    <div class="form-group">
                        ...
                        <input readonly value="0" type="text" class="form-control" id="bobot_subkriteria">
...
<script>
    ...
    function tambah() {
        ...
        $.ajax({
            method: 'POST',
            url: '<?= base_url('Subkriteria_danru_anggota/tambah')  ?>',
            data: {
    ...
    function tampil_id(id) {
        $.ajax({
            method: 'POST',
            url: '<?= base_url('Subkriteria_danru_anggota/tampil_id')  ?>',
            data: {
                id: id,
            },
            dataType: 'json',
            success: function(data) {
                if (data.status == 'berhasil') {
                    $('#id_subkriteria_edt').val(data.id_subkriteria_da)
                    $('#kode_subkriteria_edt').val(data.kode_subkriteria_da)
                    $('#nama_subkriteria_edt').val(data.nama_subkriteria_da)
                    id_kriteria = data.nama_kriteria
                    $('#bobot_subkriteria_edt').val(data.bobot_subkriteria_da)
    ...
    function ubah() {
        ...
        $.ajax({
            method: 'POST',
            url: '<?= base_url('Subkriteria_danru_anggota/ubah')  ?>',
            data: {
    ...
    function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    method: 'POST',
                    url: '<?= base_url('Subkriteria_danru_anggota/hapus')  ?>',
                    data: {
                        id: id,
                    },
                    dataType: 'json',
